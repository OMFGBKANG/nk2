...
static void mcs6000_work_ng(struct work_struct *work)
{
  ...
  //What is done here?
  struct mcs6000_ts_device *dev = container_of(to_delayed_work(work), struct mcs6000_ts_device, work);
  ...
  if ( i2c_smbus_read_i2c_block_data(dev->client, MCS6000_TS_INPUT_INFO, READ_NUM, read_buf) < 0)
    {
      ...
      queue_delayed_work(dev->ts_wq, &dev->work, (HZ / TS_SAMPLERATE_HZ) );
    }
...
static int mcs6000_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  INIT_DELAYED_WORK(&dev->work, mcs6000_work_ng);
